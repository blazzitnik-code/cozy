...
export function expandEvents(events, rangeStart, rangeEnd) {
  ...
  // Recurring series (weekly/monthly/yearly/custom) never show earlier than
  // the Monday of the CURRENT real week, even if their own event_date is
  // further in the past — so adding "every Monday" this Wednesday doesn't
  // retroactively fill in every past Monday since whenever event_date
  // happens to be set. A one-off ('once') event is unaffected: a past single
  // event is just history and should still show when browsing back.
  const thisWeekMon = new Date();
  thisWeekMon.setHours(0, 0, 0, 0);
  thisWeekMon.setDate(thisWeekMon.getDate() - ((thisWeekMon.getDay() + 6) % 7));

  for (const ev of events) {
    if (!ev.event_date) continue;
    const base = localDateFromStr(ev.event_date); // local midnight of the series start
    const earliest = ev.recurrence !== 'once' && base < thisWeekMon ? thisWeekMon : base;
    const skips = new Set(ev.skip_dates || []);

    const pushIf = (d) => {
      if (d < startDay || d > endDay || d < earliest) return;
...
// All-day blocks (a multi-day "annual leave" marker, say) are ALSO kept in
// their own group even within the same source — otherwise a 3-day all-day
// entry swallows every real, timed meeting underneath it into one "All day"
// pill, hiding exactly the "when does work end today" info this exists for.
export function mergeBusyBlocks(blocks) {
  const groups = new Map();
  for (const b of blocks) {
    const key = `${b.user_id}||${b.source_label || ''}||${b.all_day ? 'allday' : 'timed'}`;
