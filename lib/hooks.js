...
export function useArchived(householdId) {
  ...
  const updateArchived = async (id, updates) => {
    await supabase.from('archived').update(updates).eq('id', id);
  };

  const deleteArchived = async (id) => {
    await supabase.from('archived').delete().eq('id', id);
  };
  ...
  const unarchiveItem = async (archivedItem) => {
    ...
    await supabase.from('archived').delete().eq('id', archivedItem.id);
  };
...
export function useShoppingArchived(householdId) {
  ...
  const archiveChecked = async (checkedItems) => {
    ...
    await supabase.from('shopping_items').delete().in('id', checkedItems.map(i => i.id));
  };
...
export function useShoppingStores(householdId) {
  ...
  const deleteStore = async (id) => {
    await supabase.from('shopping_stores').delete().eq('id', id);
  };

  const updateStore = async (id, updates) => {
    await supabase.from('shopping_stores').update(updates).eq('id', id);
  };
...
export function useCalendarConnections(householdId, userId) {
  ...
  const saveConnection = async ({ accessToken, expiresIn, email }) => {
    ...
    await supabase.from('calendar_connections')
      .delete().eq('user_id', userId).eq('household_id', householdId);
  ...
  const saveEvents = useCallback(async (googleEvents, dateStr) => {
    ...
    await supabase.from('calendar_events')
      .delete().eq('user_id', userId).eq('event_date', dateStr);
...
export function useTodoLists(householdId) {
  ...
  const addList = async (list) => { await supabase.from('todo_lists').insert([{ ...list, household_id: householdId }]); await fetch(); };
  const updateList = async (id, updates) => { await supabase.from('todo_lists').update(updates).eq('id', id); await fetch(); };
  const archiveList = async (id) => { await supabase.from('todo_lists').update({ archived_at: new Date().toISOString() }).eq('id', id); await fetch(); };
  const deleteList = async (id) => { await supabase.from('todo_lists').delete().eq('id', id); await fetch(); };
...
export function useTodoArchivedLists(householdId) {
  ...
  const unarchiveList = async (id) => { await supabase.from('todo_lists').update({ archived_at: null }).eq('id', id); await fetch(); };
...
export function useHomeSettings(householdId, userId) {
  ...
  const saveSettings = async (updates) => {
    const { data } = await supabase.from('home_settings').upsert({ household_id: householdId, user_id: userId, ...updates, updated_at: new Date().toISOString() }, { onConflict: 'household_id,user_id' }).select().single();
    if (data) setSettings(data);
  };
...
export function useTodoItems(householdId, listId) {
  ...
  const addItem = async (title) => {
    ...
    await supabase.from('todo_items').insert([{ title, list_id: listId, household_id: householdId, checked: false, sort_order: maxOrder + 1 }]);
    await fetch();
  };
  const updateItem = async (id, updates) => { await supabase.from('todo_items').update(updates).eq('id', id); await fetch(); };
  const deleteItem = async (id) => { await supabase.from('todo_items').delete().eq('id', id); await fetch(); };
