...
const run = async (query, msg) => {
  ...
  if (error) { console.error(msg, error); notifyError(msg); }
};
...
export function useItems(householdId) {
  ...
  const addItem = async (item) => {
    await run(supabase.from('items').insert([{ ...item, household_id: householdId }]), 'Dodajanje ni uspelo');
  };
  const updateItem = async (id, updates) => {
    await run(supabase.from('items').update(updates).eq('id', id), 'Shranjevanje ni uspelo');
  };
  const deleteItem = async (id) => {
    await run(supabase.from('items').delete().eq('id', id), 'Brisanje ni uspelo');
  };
...
export function useArchived(householdId) {
  ...
  const archiveItem = async (item, wasted = false) => {
    await run(supabase.from('archived').insert([{
    ...
    }]), 'Arhiviranje ni uspelo');
    await run(supabase.from('items').delete().eq('id', item.id), 'Arhiviranje ni uspelo');
  };

  const updateArchived = async (id, updates) => {
    await run(supabase.from('archived').update(updates).eq('id', id), 'Shranjevanje ni uspelo');
  };

  const deleteArchived = async (id) => {
    await run(supabase.from('archived').delete().eq('id', id), 'Brisanje ni uspelo');
  };

  const unarchiveItem = async (archivedItem) => {
    await run(supabase.from('items').insert([{
      name: archivedItem.name, cat: archivedItem.cat, qty: archivedItem.qty,
      packets: archivedItem.packets, label: archivedItem.label,
      frozen: archivedItem.frozen, expiry: archivedItem.expiry,
      freezer: archivedItem.freezer, sticky: false,
      household_id: householdId,
    }]), 'Vračanje ni uspelo');
    await run(supabase.from('archived').delete().eq('id', archivedItem.id), 'Vračanje ni uspelo');
  };
...
export function useFreezers(householdId) {
  ...
  const addFreezer = async (freezer) => {
    ...
    await run(supabase.from('freezers').insert([{
...
export function useShoppingItems(householdId) {
  ...
  const addItem = async (item) => {
    await run(supabase.from('shopping_items').insert([{ ...item, household_id: householdId }]), 'Dodajanje ni uspelo');
  };
  const updateItem = async (id, updates) => {
    await run(supabase.from('shopping_items').update(updates).eq('id', id), 'Shranjevanje ni uspelo');
  };
  const deleteItem = async (id) => {
    await run(supabase.from('shopping_items').delete().eq('id', id), 'Brisanje ni uspelo');
  };
...
export function useShoppingArchived(householdId) {
  ...
  const archiveChecked = async (checkedItems) => {
    ...
    await run(supabase.from('shopping_archived').insert(toArchive), 'Arhiviranje ni uspelo');
    await run(supabase.from('shopping_items').delete().in('id', checkedItems.map(i => i.id)), 'Arhiviranje ni uspelo');
  };
...
export function useShoppingFavourites(householdId) {
  ...
  const toggleFavourite = async (name) => {
    ...
    if (existing) {
      await run(supabase.from('shopping_favourites').delete().eq('id', existing.id), 'Shranjevanje ni uspelo');
    } else {
      await run(supabase.from('shopping_favourites').insert([{
...
export function useShoppingStores(householdId) {
  ...
  const addStore = async (store) => {
    ...
    await run(supabase.from('shopping_stores').insert([{
      id, name: store.name, icon: store.icon, sort_order: stores.length, household_id: householdId,
    }]), 'Dodajanje ni uspelo');
  };

  const deleteStore = async (id) => {
    await run(supabase.from('shopping_stores').delete().eq('id', id), 'Brisanje ni uspelo');
  };

  const updateStore = async (id, updates) => {
    await run(supabase.from('shopping_stores').update(updates).eq('id', id), 'Shranjevanje ni uspelo');
  };
...
export function useCalendarConnections(householdId, userId) {
  const { data: connections, loading, refetch } = useHouseholdTable(
    'calendar_connections', householdId, 'created_at', true
  );

  const saveConnection = async ({ accessToken, expiresIn, email }) => {
    const expiresAt = new Date(Date.now() + (expiresIn || 3600) * 1000).toISOString();
    await run(supabase.from('calendar_connections')
      .delete().eq('user_id', userId).eq('household_id', householdId), 'Povezava koledarja ni uspela');
    await run(supabase.from('calendar_connections').insert({
      user_id: userId,
      household_id: householdId,
      access_token: accessToken,
      expires_at: expiresAt,
      google_email: email,
    }), 'Povezava koledarja ni uspela');
    await refetch();
  };

  const removeConnection = async (id) => {
    await run(supabase.from('calendar_connections').delete().eq('id', id), 'Brisanje ni uspelo');
    await refetch();
  };

  const saveEvents = useCallback(async (googleEvents, dateStr) => {
    ...
    await run(supabase.from('calendar_events')
      .delete().eq('user_id', userId).eq('event_date', dateStr), 'Shranjevanje dogodkov ni uspelo');
    await run(supabase.from('calendar_events').insert(rows), 'Shranjevanje dogodkov ni uspelo');
  }, [householdId, userId]);
...
export function useCalendarEvents(householdId, dateStr) {
  ...
  const updateEvent = useCallback(async (id, updates) => {
    await run(supabase.from('calendar_events').update(updates).eq('id', id), 'Shranjevanje ni uspelo');
    setTick(t => t + 1);
  }, []);
...
export function useTodoLists(householdId) {
  ...
  const fetch = useCallback(async () => {
    ...
    const { data } = await supabase
      .from('todo_lists').select('*').eq('household_id', householdId)
      .is('archived_at', null).order('due_date', { ascending: true, nullsFirst: false });
  ...
  const addList = async (list) => { await run(supabase.from('todo_lists').insert([{ ...list, household_id: householdId }]), 'Dodajanje ni uspelo'); await fetch(); };
  const updateList = async (id, updates) => { await run(supabase.from('todo_lists').update(updates).eq('id', id), 'Shranjevanje ni uspelo'); await fetch(); };
  const archiveList = async (id) => { await run(supabase.from('todo_lists').update({ archived_at: new Date().toISOString() }).eq('id', id), 'Arhiviranje ni uspelo'); await fetch(); };
  const deleteList = async (id) => { await run(supabase.from('todo_lists').delete().eq('id', id), 'Brisanje ni uspelo'); await fetch(); };
...
export function useTodoArchivedLists(householdId) {
  ...
  const unarchiveList = async (id) => { await run(supabase.from('todo_lists').update({ archived_at: null }).eq('id', id), 'Vračanje ni uspelo'); await fetch(); };
...
export function useHomeSettings(householdId, userId) {
  ...
  const fetch = useCallback(async () => {
    ...
    const { data } = await supabase.from('home_settings').select('*').eq('household_id', householdId).eq('user_id', userId).single();
  ...
  const saveSettings = async (updates) => {
    const { data, error } = await supabase.from('home_settings').upsert({ household_id: householdId, user_id: userId, ...updates, updated_at: new Date().toISOString() }, { onConflict: 'household_id,user_id' }).select().single();
    if (error) { console.error('Shranjevanje nastavitev ni uspelo', error); notifyError('Shranjevanje nastavitev ni uspelo'); }
    if (data) setSettings(data);
  };
...
export function useTodoItems(householdId, listId) {
  ...
  const fetch = useCallback(async () => {
    if (!householdId || !listId) { setLoading(false); return; }
    const { data } = await supabase
      .from('todo_items').select('*').eq('list_id', listId)
  ...
  const addItem = async (title) => {
    ...
    await run(supabase.from('todo_items').insert([{ title, list_id: listId, household_id: householdId, checked: false, sort_order: maxOrder + 1 }]), 'Dodajanje ni uspelo');
    await fetch();
  };
  const updateItem = async (id, updates) => { await run(supabase.from('todo_items').update(updates).eq('id', id), 'Shranjevanje ni uspelo'); await fetch(); };
  const deleteItem = async (id) => { await run(supabase.from('todo_items').delete().eq('id', id), 'Brisanje ni uspelo'); await fetch(); };
  const toggleItem = (id) => { const item = items.find(i => i.id === id); if (item) updateItem(id, { checked: !item.checked }); };
