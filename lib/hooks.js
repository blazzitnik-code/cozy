  ...
  const saveEvents = useCallback(
    async (googleEvents, dateStr) => {
      ...
      await run(
        ...
        'Shranjevanje dogodkov ni uspelo',
      );
...
export function useTodoLists(householdId) {
  ...
  const addList = async (list) => {
    await run(supabase.from('todo_lists').insert([{ ...list, household_id: householdId }]), 'Dodajanje ni uspelo');
    await fetch();
  };
  const updateList = async (id, updates) => {
    await run(supabase.from('todo_lists').update(updates).eq('id', id), 'Shranjevanje ni uspelo');
    await fetch();
  };
  ...
  const deleteList = async (id) => {
    await run(supabase.from('todo_lists').delete().eq('id', id), 'Brisanje ni uspelo');
    await fetch();
  };
...
export function useTodoArchivedLists(householdId) {
  ...
  const unarchiveList = async (id) => {
    await run(supabase.from('todo_lists').update({ archived_at: null }).eq('id', id), 'Vračanje ni uspelo');
    await fetch();
  };
...
export function useHomeSettings(householdId, userId) {
  ...
  const saveSettings = async (updates) => {
    const { data, error } = await supabase
      ...
      .single();
    if (error) {
      console.error('Shranjevanje nastavitev ni uspelo', error);
      notifyError('Shranjevanje nastavitev ni uspelo');
    }
    if (data) setSettings(data);
  };
...
export function useTodoItems(householdId, listId) {
  ...
  const updateItem = async (id, updates) => {
    await run(supabase.from('todo_items').update(updates).eq('id', id), 'Shranjevanje ni uspelo');
    await fetch();
  };
  const deleteItem = async (id) => {
    await run(supabase.from('todo_items').delete().eq('id', id), 'Brisanje ni uspelo');
    await fetch();
  };
  const toggleItem = (id) => {
