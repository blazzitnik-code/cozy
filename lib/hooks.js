'use client';
import { useState, useEffect, useCallback, useMemo, useRef, useId } from 'react';
import { supabase } from './supabase';
import { notifyError } from './notify';

// Reports failed writes to the user via toast (see <Toaster /> in ui.js).
// `msg` is an Errors.* message key — Toaster translates it at render time.
// Returns whether the write succeeded — two-step "moves" (insert then delete)
// must abort the delete when the insert failed, or the row is lost.
// Reads keep their per-hook handling.
const run = async (query, msg) => {
  const { error } = await query;
  if (error) {
    console.error(msg, error);
    notifyError(msg);
  }
  return !error;
};

// ─── SEARCH NORMALIZATION ───
export const normalizujNiz = (niz) => {
  if (!niz) return '';
  return niz
    .toLowerCase()
    .normalize('NFKD')
    .replace(/[\u0300-\u036f]/g, '')
    .replace(/[šŝ]/g, 's')
    .replace(/[čć]/g, 'c')
    .replace(/[žź]/g, 'z');
};

// ─── AUTH ───
export function useAuth() {
  const [user, setUser] = useState(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    supabase.auth.getSession().then(({ data: { session } }) => {
      setUser(session?.user || null);
      setLoading(false);
    });
    const {
      data: { subscription },
    } = supabase.auth.onAuthStateChange((_event, session) => {
      setUser(session?.user || null);
    });
    return () => subscription.unsubscribe();
  }, []);

  const signInWithGoogle = async () => {
    const redirectUrl = typeof window !== 'undefined' ? window.location.origin : '';
    await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: { redirectTo: redirectUrl },
    });
  };

  // Dev-only path for LAN-device testing (Google OAuth can't round-trip to a
  // private-IP local stack); the login form gates itself on NODE_ENV.
  const signInWithPassword = async (email, password) => {
    const { error } = await supabase.auth.signInWithPassword({ email, password });
    if (error) throw error;
  };

  const signOut = async () => {
    await supabase.auth.signOut();
  };

  return { user, loading, signInWithGoogle, signInWithPassword, signOut };
}

// ─── HOUSEHOLD ───
export function useHousehold(user) {
  const [household, setHousehold] = useState(null);
  const [members, setMembers] = useState([]);
  // Which user the current household/members state was resolved for:
  // undefined until the first fetch settles, then user.id or null.
  const [fetchedFor, setFetchedFor] = useState();
  const retryTimer = useRef(null);

  const fetch = useCallback(async () => {
    if (!user) {
      setHousehold(null);
      setFetchedFor(null);
      return;
    }

    // One embedded select instead of three sequential round trips; RLS
    // (is_household_member) scopes both tables to the user's household.
    const { data, error } = await supabase
      .from('households')
      .select('id, name, join_code, household_members(*)')
      .limit(1)
      .maybeSingle();

    // A transient fetch error must not read as "no household" — page.js would
    // flash the create-household screen for an existing member. Stay loading
    // (fetchedFor untouched) and retry.
    if (error) {
      console.error('household fetch failed', error);
      clearTimeout(retryTimer.current);
      retryTimer.current = setTimeout(fetch, 4000);
      return;
    }

    if (data) {
      const { household_members: allMembers, ...hh } = data;
      setHousehold(hh);
      setMembers(allMembers ?? []);
    } else {
      setHousehold(null);
    }
    setFetchedFor(user.id);
  }, [user]);

  // Derived during render, not stored: the hook first runs with user=null
  // (auth still resolving) and a stored flag would report loading=false for
  // one frame after the user arrives (effects run post-paint) — page.js
  // would flash the create-household screen while the query is in flight.
  const loading = fetchedFor === undefined || fetchedFor !== (user?.id ?? null);

  useEffect(() => {
    fetch();
    return () => clearTimeout(retryTimer.current);
  }, [fetch]);

  const createHousehold = async (name, displayName) => {
    const { data, error } = await supabase.rpc('create_household', {
      p_name: name,
      p_user_id: user.id,
      p_display_name: displayName,
    });
    if (error) throw error;
    await fetch();
    return data;
  };

  const joinHousehold = async (code, displayName) => {
    const { data, error } = await supabase.rpc('join_household', {
      p_code: code.toUpperCase(),
      p_user_id: user.id,
      p_display_name: displayName,
    });
    if (error) throw error;
    await fetch();
    return data;
  };

  return { household, members, loading, createHousehold, joinHousehold, refetch: fetch };
}

// ─── GENERIC HOUSEHOLD-SCOPED TABLE HOOK ───
// Optimistic overrides: `applyLocal(id, patch)` stashes field changes that are
// merged ON TOP of whatever a fetch returns, so an optimistic toggle can't
// flicker even if an unrelated realtime refetch lands while the write is still
// in flight. An override is dropped only once a fetch comes back where the
// server row already matches it (write confirmed) or the row is gone; a failed
// write reverts it via applyLocal(id, null).
function useHouseholdTable(tableName, householdId, orderBy, ascending) {
  // Own realtime channel per HOOK INSTANCE, not just per table+household —
  // two components can legitimately read the same table for the same
  // household at once (e.g. MELCloud's and Vaillant's useProviderConnection
  // both reading provider_connections). Supabase reuses an existing channel
  // object for an identical topic name instead of creating an independent
  // one, so a shared name here made the second instance's .on() call throw
  // "cannot add postgres_changes callbacks ... after subscribe()" — it was
  // registering a listener on a channel the first instance had already
  // subscribed. useId() keeps the topic unique per mount regardless of how
  // many instances read the same table+household combo.
  const instanceId = useId();
  const [data, setData] = useState([]);
  const [overrides, setOverrides] = useState({}); // id → partial patch
  const [loading, setLoading] = useState(true);

  const fetch = useCallback(async () => {
    if (!householdId) {
      setLoading(false);
      return;
    }
    const { data: rows } = await supabase
      .from(tableName)
      .select('*')
      .eq('household_id', householdId)
      .order(orderBy, { ascending });
    if (rows) {
      setData(rows);
      // Reconcile overrides against fresh server truth.
      setOverrides((ov) => {
        const ids = Object.keys(ov);
        if (!ids.length) return ov;
        let changed = false;
        const next = {};
        for (const id of ids) {
          const row = rows.find((r) => r.id === id);
          // Keep the override only while the row exists AND the server hasn't
          // caught up to every patched field yet.
          if (row && !Object.entries(ov[id]).every(([k, v]) => row[k] === v)) next[id] = ov[id];
          else changed = true;
        }
        return changed ? next : ov;
      });
    }
    setLoading(false);
  }, [householdId, tableName, orderBy, ascending]);

  useEffect(() => {
    fetch();
    // Bulk writes arrive as one realtime message per row — coalesce the burst
    // into a single trailing refetch instead of N full-table fetches.
    let debounce;
    const ch = supabase
      .channel(tableName + '-' + householdId + '-' + instanceId)
      .on('postgres_changes', { event: '*', schema: 'public', table: tableName }, () => {
        clearTimeout(debounce);
        debounce = setTimeout(fetch, 200);
      })
      .subscribe();
    return () => {
      clearTimeout(debounce);
      supabase.removeChannel(ch);
    };
  }, [fetch, tableName, householdId, instanceId]);

  // patch === null reverts (drops) the row's override.
  const applyLocal = useCallback((id, patch) => {
    setOverrides((ov) => {
      if (patch === null) {
        if (!(id in ov)) return ov;
        const next = { ...ov };
        delete next[id];
        return next;
      }
      return { ...ov, [id]: { ...ov[id], ...patch } };
    });
  }, []);

  const merged = useMemo(
    () =>
      Object.keys(overrides).length ? data.map((r) => (overrides[r.id] ? { ...r, ...overrides[r.id] } : r)) : data,
    [data, overrides],
  );

  return { data: merged, loading, refetch: fetch, applyLocal };
}

// Fire-and-forget write for optimistic UI: the local state is already updated
// (applyLocal) so the UI is instant; on error we revert that row and toast.
const optimisticWrite = (query, id, applyLocal, msg) => {
  Promise.resolve(query).then(({ error }) => {
    if (error) {
      console.error(msg, error);
      notifyError(msg);
      applyLocal(id, null);
    }
  });
};

// ─── FREEZER ITEMS ───
export function useItems(householdId) {
  const { data: items, loading, refetch } = useHouseholdTable('items', householdId, 'expiry', true);

  const addItem = async (item) => {
    await run(supabase.from('items').insert([{ ...item, household_id: householdId }]), 'Errors.addFailed');
  };
  const updateItem = async (id, updates) => {
    await run(supabase.from('items').update(updates).eq('id', id), 'Errors.saveFailed');
  };
  const deleteItem = async (id) => {
    await run(supabase.from('items').delete().eq('id', id), 'Errors.deleteFailed');
  };

  return { items, loading, addItem, updateItem, deleteItem, refetch };
}

// ─── ARCHIVED ───
export function useArchived(householdId) {
  const { data: archived, loading, refetch } = useHouseholdTable('archived', householdId, 'archived_at', false);

  const archiveItem = async (item, wasted = false) => {
    const ok = await run(
      supabase.from('archived').insert([
        {
          name: item.name,
          cat: item.cat,
          qty: item.qty,
          packets: item.packets,
          label: item.label,
          frozen: item.frozen,
          expiry: item.expiry,
          freezer: item.freezer,
          wasted,
          household_id: householdId,
        },
      ]),
      'Errors.archiveFailed',
    );
    if (!ok) return;
    await run(supabase.from('items').delete().eq('id', item.id), 'Errors.archiveFailed');
  };

  const updateArchived = async (id, updates) => {
    await run(supabase.from('archived').update(updates).eq('id', id), 'Errors.saveFailed');
  };

  const deleteArchived = async (id) => {
    await run(supabase.from('archived').delete().eq('id', id), 'Errors.deleteFailed');
  };

  const unarchiveItem = async (archivedItem) => {
    const ok = await run(
      supabase.from('items').insert([
        {
          name: archivedItem.name,
          cat: archivedItem.cat,
          qty: archivedItem.qty,
          packets: archivedItem.packets,
          label: archivedItem.label,
          frozen: archivedItem.frozen,
          expiry: archivedItem.expiry,
          freezer: archivedItem.freezer,
          sticky: false,
          household_id: householdId,
        },
      ]),
      'Errors.restoreFailed',
    );
    if (!ok) return;
    await run(supabase.from('archived').delete().eq('id', archivedItem.id), 'Errors.restoreFailed');
  };

  return { archived, loading, archiveItem, updateArchived, deleteArchived, unarchiveItem, refetch };
}

// ─── FREEZERS ───
export function useFreezers(householdId) {
  const { data: freezers, loading, refetch } = useHouseholdTable('freezers', householdId, 'sort_order', true);

  const addFreezer = async (freezer) => {
    const id = freezer.name.toLowerCase().replace(/\s+/g, '_') + '_' + Date.now();
    await run(
      supabase.from('freezers').insert([
        {
          id,
          name: freezer.name,
          icon: freezer.icon,
          sort_order: freezers.length,
          household_id: householdId,
        },
      ]),
      'Errors.addFailed',
    );
  };
  // Composite PK (household_id, id) — filter on both.
  const updateFreezer = async (id, updates) => {
    await run(
      supabase.from('freezers').update(updates).eq('household_id', householdId).eq('id', id),
      'Errors.saveFailed',
    );
  };
  const deleteFreezer = async (id) => {
    await run(supabase.from('freezers').delete().eq('household_id', householdId).eq('id', id), 'Errors.deleteFailed');
  };

  return { freezers, addFreezer, updateFreezer, deleteFreezer, refetch };
}

// ─── CATEGORIES ───
export function useCategories(householdId) {
  const [categories, setCategories] = useState({});
  const [loading, setLoading] = useState(true);

  const fetch = useCallback(async () => {
    if (!householdId) {
      setLoading(false);
      return;
    }
    const { data } = await supabase
      .from('categories')
      .select('*')
      .or('household_id.eq.' + householdId + ',household_id.is.null');
    if (data) {
      const map = {};
      data.forEach((c) => {
        map[c.id] = { label: c.label, icon: c.icon, color: c.color, months: c.months };
      });
      setCategories(map);
    }
    setLoading(false);
  }, [householdId]);

  useEffect(() => {
    fetch();
    let debounce;
    const ch = supabase
      .channel('categories-' + householdId)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'categories' }, () => {
        clearTimeout(debounce);
        debounce = setTimeout(fetch, 200);
      })
      .subscribe();
    return () => {
      clearTimeout(debounce);
      supabase.removeChannel(ch);
    };
  }, [fetch, householdId]);

  const addCategory = async (cat) => {
    const id = cat.label.toLowerCase().replace(/\s+/g, '_') + '_' + Date.now();
    await run(
      supabase.from('categories').insert([
        {
          id,
          label: cat.label,
          icon: cat.icon,
          color: cat.color,
          months: cat.months,
          household_id: householdId,
        },
      ]),
      'Errors.addFailed',
    );
  };

  return { categories, loading, addCategory, refetch: fetch };
}

// ─── SHOPPING ITEMS ───
export function useShoppingItems(householdId) {
  const {
    data: items,
    loading,
    refetch,
    applyLocal,
  } = useHouseholdTable('shopping_items', householdId, 'created_at', false);

  const addItem = async (item) => {
    await run(supabase.from('shopping_items').insert([{ ...item, household_id: householdId }]), 'Errors.addFailed');
  };
  // Optimistic: instant local check/qty/reorder, write in the background.
  const updateItem = useCallback(
    (id, updates) => {
      applyLocal(id, updates);
      optimisticWrite(
        supabase.from('shopping_items').update(updates).eq('id', id),
        id,
        applyLocal,
        'Errors.saveFailed',
      );
    },
    [applyLocal],
  );
  const deleteItem = async (id) => {
    await run(supabase.from('shopping_items').delete().eq('id', id), 'Errors.deleteFailed');
  };

  return { items, loading, addItem, updateItem, deleteItem, refetch };
}

// ─── SHOPPING ARCHIVED ───
export function useShoppingArchived(householdId) {
  const {
    data: archived,
    loading,
    refetch,
  } = useHouseholdTable('shopping_archived', householdId, 'completed_at', false);

  // meta (optional, from the checkout modal): { storeNote, amount }.
  // purchase_group links this checkout's items; store_note goes on all of them,
  // amount only on the first row so totals never double-count.
  const archiveChecked = async (checkedItems, meta = {}) => {
    const group = crypto.randomUUID();
    const storeNote = meta.storeNote?.trim() || null;
    const amount = meta.amount != null && !isNaN(meta.amount) ? meta.amount : null;
    const completedAt = new Date().toISOString();
    const toArchive = checkedItems.map((i, idx) => ({
      name: i.name,
      qty: i.qty || '',
      category: i.category || '',
      store: i.store || '',
      household_id: householdId,
      completed_at: completedAt, // explicit ISO — don't rely on a DB default
      purchase_group: group,
      store_note: storeNote,
      amount: idx === 0 ? amount : null,
    }));
    const ok = await run(supabase.from('shopping_archived').insert(toArchive), 'Errors.archiveFailed');
    if (!ok) return;
    await run(
      supabase
        .from('shopping_items')
        .delete()
        .in(
          'id',
          checkedItems.map((i) => i.id),
        ),
      'Errors.archiveFailed',
    );
  };

  // Retroactively fill in the store/amount for a purchase archived without
  // one (the analysis screen offers this wherever a total is missing —
  // opt-in-at-checkout means most purchases never get a price otherwise).
  // Same write shape as archiveChecked: store_note on every row in the
  // group, amount on exactly one so a raw sum() over the table never
  // double-counts a single checkout.
  const updatePurchaseAmount = async (groupItems, meta = {}) => {
    const ids = groupItems.map((i) => i.id);
    if (!ids.length) return false;
    const storeNote = meta.storeNote?.trim() || null;
    const amount = meta.amount != null && !isNaN(meta.amount) ? meta.amount : null;
    const ok1 = await run(
      supabase.from('shopping_archived').update({ store_note: storeNote }).in('id', ids),
      'Errors.purchaseUpdateFailed',
    );
    const ok2 = await run(
      supabase.from('shopping_archived').update({ amount }).eq('id', ids[0]),
      'Errors.purchaseUpdateFailed',
    );
    if (ids.length > 1) {
      await run(
        supabase.from('shopping_archived').update({ amount: null }).in('id', ids.slice(1)),
        'Errors.purchaseUpdateFailed',
      );
    }
    await refetch();
    return ok1 && ok2;
  };

  // Deletes an entire checkout (every shopping_archived row sharing the
  // same purchase trip) — e.g. a test purchase entered while trying out
  // the checkout-amount feature, not a real everyday "undo my shopping"
  // action. No UI currently exposes single-item deletion on purpose: a
  // 3-item checkout with the amount on item 1 (see archiveChecked above)
  // would otherwise leave the remaining 2 items "orphaned" with no amount
  // if only one row were removed.
  const deletePurchase = async (ids) => {
    if (!ids?.length) return false;
    const ok = await run(supabase.from('shopping_archived').delete().in('id', ids), 'Errors.deleteFailed');
    if (ok) await refetch();
    return ok;
  };

  return { archived, loading, archiveChecked, updatePurchaseAmount, deletePurchase, refetch };
}

// ─── SHOPPING FAVOURITES ───
export function useShoppingFavourites(householdId) {
  const {
    data: favourites,
    loading,
    refetch,
  } = useHouseholdTable('shopping_favourites', householdId, 'use_count', false);

  const toggleFavourite = async (name) => {
    const existing = favourites.find((f) => f.name.toLowerCase() === name.toLowerCase());
    if (existing) {
      await run(supabase.from('shopping_favourites').delete().eq('id', existing.id), 'Errors.saveFailed');
    } else {
      await run(
        supabase.from('shopping_favourites').insert([
          {
            name,
            household_id: householdId,
            use_count: 1,
            last_used: new Date().toISOString(),
          },
        ]),
        'Errors.saveFailed',
      );
    }
  };

  return { favourites, toggleFavourite, refetch };
}

// ─── SHOPPING STORES ───
// ─── SHOPPING SECTION ORDER ───
// Household-level override for the order of the "grouped by category"
// sections in Trgovko (see detectCategory() in ShoppingModule.js for the
// hardcoded default order/keys). "drugo" (catch-all) is deliberately never
// stored here - it always renders last. Rows are sparse: a household that
// never opened "Uredi sekcije" has none, and callers fall back to the
// hardcoded default order for any key missing here.
export function useShoppingSections(householdId) {
  const { data: sections, loading, refetch } = useHouseholdTable('shopping_sections', householdId, 'sort_order', true);

  // Persists a full explicit order for the given keys (upserts every key,
  // not just the ones that moved) so the effective order is fully explicit
  // going forward — called after each row's drag ends in the reorder modal
  // (ManageSectionsModal), same as ShopGroup persists item order per drag.
  const reorderSections = async (orderedKeys) => {
    await run(
      supabase.from('shopping_sections').upsert(
        orderedKeys.map((id, i) => ({ id, household_id: householdId, sort_order: i })),
        { onConflict: 'household_id,id' },
      ),
      'Errors.saveFailed',
    );
  };

  return { sections, loading, refetch, reorderSections };
}

export function useShoppingStores(householdId) {
  const { data: stores, loading, refetch } = useHouseholdTable('shopping_stores', householdId, 'sort_order', true);

  const addStore = async (store) => {
    const id = store.name.toLowerCase().replace(/\s+/g, '_') + '_' + Date.now();
    await run(
      supabase.from('shopping_stores').insert([
        {
          id,
          name: store.name,
          icon: store.icon,
          sort_order: stores.length,
          household_id: householdId,
        },
      ]),
      'Errors.addFailed',
    );
  };

  const deleteStore = async (id) => {
    await run(supabase.from('shopping_stores').delete().eq('id', id), 'Errors.deleteFailed');
  };

  const updateStore = async (id, updates) => {
    await run(supabase.from('shopping_stores').update(updates).eq('id', id), 'Errors.saveFailed');
  };

  return { stores, addStore, updateStore, deleteStore, refetch };
}

// ─── CALENDAR CONNECTIONS ───
export function useCalendarConnections(householdId, userId) {
  const {
    data: connections,
    loading,
    refetch,
  } = useHouseholdTable('calendar_connections', householdId, 'created_at', true);

  // Memoized so consumers (AppShell's connectCalendar + its token-refresh
  // timer effect) keep a stable identity across realtime-triggered renders.
  const saveConnection = useCallback(
    async ({ accessToken, expiresIn, email }) => {
      const expiresAt = new Date(Date.now() + (expiresIn || 3600) * 1000).toISOString();
      await run(
        supabase.from('calendar_connections').delete().eq('user_id', userId).eq('household_id', householdId),
        'Errors.calendarConnectFailed',
      );
      await run(
        supabase.from('calendar_connections').insert({
          user_id: userId,
          household_id: householdId,
          access_token: accessToken,
          expires_at: expiresAt,
          google_email: email,
        }),
        'Errors.calendarConnectFailed',
      );
      await refetch();
    },
    [householdId, userId, refetch],
  );

  const removeConnection = useCallback(
    async (id) => {
      await run(supabase.from('calendar_connections').delete().eq('id', id), 'Errors.deleteFailed');
      await refetch();
    },
    [refetch],
  );

  // Marks the own row as expired (Google rejected the stored token with a 401
  // despite a future expires_at) so isConnected flips false and the UI falls
  // back to the connect button instead of retrying forever.
  const expireConnection = useCallback(async () => {
    await run(
      supabase
        .from('calendar_connections')
        .update({ expires_at: new Date().toISOString() })
        .eq('user_id', userId)
        .eq('household_id', householdId),
      'Errors.saveFailed',
    );
    await refetch();
  }, [householdId, userId, refetch]);

  const saveEvents = useCallback(
    async (googleEvents, dateStr) => {
      const rows = googleEvents.map((ev) => ({
        household_id: householdId,
        user_id: userId,
        google_event_id: ev.id,
        // Untitled events are stored with an empty title; the UI falls back to
        // the viewer-locale translation via eventTitle() in lib/intl.js.
        title: ev.summary || '',
        start_time: ev.start?.dateTime || ev.start?.date || null,
        end_time: ev.end?.dateTime || ev.end?.date || null,
        is_all_day: !!ev.start?.date,
        location: ev.location || null,
        event_date: dateStr,
      }));
      // Upsert keyed on (user_id, event_date, google_event_id) — the payload
      // omits label/is_important, so annotations survive every re-sync.
      if (rows.length) {
        const ok = await run(
          supabase.from('calendar_events').upsert(rows, { onConflict: 'user_id,event_date,google_event_id' }),
          'Errors.eventsSaveFailed',
        );
        if (!ok) return; // don't wipe the day if the write failed
      }
      // Drop events removed on the Google side (all of them on an empty day).
      let stale = supabase.from('calendar_events').delete().eq('user_id', userId).eq('event_date', dateStr);
      if (rows.length) {
        const ids = rows.map((r) => '"' + String(r.google_event_id).replaceAll('"', '') + '"').join(',');
        stale = stale.not('google_event_id', 'in', '(' + ids + ')');
      }
      await run(stale, 'Errors.eventsSaveFailed');
    },
    [householdId, userId],
  );

  const myConnection = connections.find((c) => c.user_id === userId);
  const isConnected = !!myConnection && new Date(myConnection.expires_at) > new Date();

  return {
    connections,
    myConnection,
    isConnected,
    loading,
    saveConnection,
    removeConnection,
    expireConnection,
    saveEvents,
    refetch,
  };
}

// ─── CALENDAR EVENTS (manual Koledarko, whole household) ───
// Loads every raw event once (realtime-refetched); recurrence is expanded in
// the UI via expandEvents(). Google sync is phase 2 — this schema is manual.
export function useCalendarEvents(householdId) {
  const { data: events, loading, refetch } = useHouseholdTable('calendar_events', householdId, 'event_date', true);

  const addEvent = useCallback(
    async (ev) => {
      await run(supabase.from('calendar_events').insert([{ ...ev, household_id: householdId }]), 'Errors.addFailed');
    },
    [householdId],
  );

  const updateEvent = useCallback(async (id, updates) => {
    await run(supabase.from('calendar_events').update(updates).eq('id', id), 'Errors.saveFailed');
  }, []);

  const deleteEvent = useCallback(async (id) => {
    await run(supabase.from('calendar_events').delete().eq('id', id), 'Errors.deleteFailed');
  }, []);

  // Skip a single occurrence of a recurring series: append the date to
  // skip_dates (the whole series stays; expandEvents() drops that one).
  const skipOccurrence = useCallback(
    async (id, dateStr) => {
      const ev = events.find((e) => e.id === id);
      if (!ev) return;
      const skips = [...(ev.skip_dates || []), dateStr];
      await run(supabase.from('calendar_events').update({ skip_dates: skips }).eq('id', id), 'Errors.saveFailed');
    },
    [events],
  );

  return { events, loading, refetch, addEvent, updateEvent, deleteEvent, skipOccurrence };
}

// ─── CALENDAR FREEBUSY SOURCES (own ICS URLs, private) ───
// RLS on calendar_freebusy_sources restricts SELECT to user_id = auth.uid(),
// so this naturally only ever returns the caller's own sources even though
// the query filters by household_id like every other useHouseholdTable
// consumer — the partner's rows are invisible at the database layer, not
// just hidden in the UI. Sync (fetching the ICS + computing busy blocks)
// happens server-side on a cron; this hook only manages the source list.
export function useFreebusySources(householdId, userId) {
  const {
    data: sources,
    loading,
    refetch,
  } = useHouseholdTable('calendar_freebusy_sources', householdId, 'created_at', true);

  const addSource = useCallback(
    async ({ label, icsUrl }) => {
      await run(
        supabase
          .from('calendar_freebusy_sources')
          .insert({ household_id: householdId, user_id: userId, label: label || null, ics_url: icsUrl }),
        'Errors.freebusyAddFailed',
      );
      await refetch();
    },
    [householdId, userId, refetch],
  );

  const removeSource = useCallback(
    async (id) => {
      await run(supabase.from('calendar_freebusy_sources').delete().eq('id', id), 'Errors.deleteFailed');
      await refetch();
    },
    [refetch],
  );

  const updateSourceLabel = useCallback(
    async (id, label) => {
      const ok = await run(
        supabase
          .from('calendar_freebusy_sources')
          .update({ label: label || null })
          .eq('id', id),
        'Errors.freebusyAddFailed',
      );
      await refetch();
      return ok;
    },
    [refetch],
  );

  // Manual "Sync now" — fires the same sync-freebusy edge function the
  // 30-min cron already calls (see app/api/calendar/sync-freebusy), for
  // when a source's error needs a faster retry than waiting for the next
  // tick. Fire-and-forget from the UI's perspective: the function updates
  // last_synced_at/last_error per source, which arrive back here via the
  // realtime subscription already wired into useHouseholdTable, so no
  // refetch() call is needed once it settles.
  const [syncing, setSyncing] = useState(false);
  const syncNow = useCallback(async () => {
    const {
      data: { session },
    } = await supabase.auth.getSession();
    if (!session) return false;

    setSyncing(true);
    try {
      const res = await fetch('/api/calendar/sync-freebusy', {
        method: 'POST',
        headers: { Authorization: `Bearer ${session.access_token}` },
      });
      return res.ok;
    } catch {
      return false;
    } finally {
      setSyncing(false);
    }
  }, []);

  return { sources, loading, refetch, addSource, removeSource, updateSourceLabel, syncNow, syncing };
}

// ─── CALENDAR BUSY BLOCKS (computed from freebusy sources, whole household) ───
// Read-only from the client — only the sync-freebusy edge function
// (service_role) ever writes here, so there's no add/update/delete here.
// Range-filtered so Koledarko's views don't pull the entire 60-day sync
// window on every render; callers pass ISO timestamp strings.
export function useBusyBlocks(householdId, rangeStartStr, rangeEndStr) {
  const [blocks, setBlocks] = useState([]);
  const [loading, setLoading] = useState(true);

  const fetch = useCallback(async () => {
    if (!householdId || !rangeStartStr || !rangeEndStr) {
      setLoading(false);
      return;
    }
    const { data } = await supabase
      .from('calendar_busy_blocks')
      .select('*')
      .eq('household_id', householdId)
      .lte('starts_at', rangeEndStr)
      .gte('ends_at', rangeStartStr)
      .order('starts_at', { ascending: true });
    if (data) setBlocks(data);
    setLoading(false);
  }, [householdId, rangeStartStr, rangeEndStr]);

  useEffect(() => {
    fetch();
    // Coalesce realtime bursts (a sync can rewrite many rows at once) into a
    // single trailing refetch instead of N range queries.
    let debounce;
    const ch = supabase
      .channel('calendar_busy_blocks-' + householdId)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'calendar_busy_blocks' }, () => {
        clearTimeout(debounce);
        debounce = setTimeout(fetch, 200);
      })
      .subscribe();
    return () => {
      clearTimeout(debounce);
      supabase.removeChannel(ch);
    };
  }, [fetch, householdId]);

  return { blocks, loading, refetch: fetch };
}

// ─── TODO LISTS (active + archived from one query/channel) ───
export function useTodoLists(householdId) {
  const [rows, setRows] = useState([]);
  const [loading, setLoading] = useState(true);
  const chName = useRef('todo_lists-' + householdId + '-' + Date.now()).current;

  const fetch = useCallback(async () => {
    if (!householdId) {
      setLoading(false);
      return;
    }
    const { data } = await supabase.from('todo_lists').select('*').eq('household_id', householdId);
    if (data) setRows(data);
    setLoading(false);
  }, [householdId]);

  useEffect(() => {
    fetch();
    let debounce;
    const ch = supabase
      .channel(chName)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'todo_lists' }, () => {
        clearTimeout(debounce);
        debounce = setTimeout(fetch, 200);
      })
      .subscribe();
    return () => {
      clearTimeout(debounce);
      supabase.removeChannel(ch);
    };
  }, [fetch, chName]);

  // Plain code-unit compares on the ISO strings — localeCompare mis-sorts
  // them under ICU collation (symbols/punctuation get lower weight).
  const lists = useMemo(
    () =>
      rows
        .filter((l) => !l.archived_at)
        .sort((a, b) => {
          // due_date asc, undated last (old `order('due_date', { nullsFirst: false })`)
          if (!a.due_date) return b.due_date ? 1 : 0;
          if (!b.due_date) return -1;
          return a.due_date < b.due_date ? -1 : a.due_date > b.due_date ? 1 : 0;
        }),
    [rows],
  );
  const archivedLists = useMemo(
    () => rows.filter((l) => l.archived_at).sort((a, b) => (a.archived_at < b.archived_at ? 1 : -1)),
    [rows],
  );

  const addList = async (list) => {
    await run(supabase.from('todo_lists').insert([{ ...list, household_id: householdId }]), 'Errors.addFailed');
    await fetch();
  };
  const updateList = async (id, updates) => {
    await run(supabase.from('todo_lists').update(updates).eq('id', id), 'Errors.saveFailed');
    await fetch();
  };
  const archiveList = async (id) => {
    await run(
      supabase.from('todo_lists').update({ archived_at: new Date().toISOString() }).eq('id', id),
      'Errors.archiveFailed',
    );
    await fetch();
  };
  const unarchiveList = async (id) => {
    await run(supabase.from('todo_lists').update({ archived_at: null }).eq('id', id), 'Errors.restoreFailed');
    await fetch();
  };
  const deleteList = async (id) => {
    await run(supabase.from('todo_lists').delete().eq('id', id), 'Errors.deleteFailed');
    await fetch();
  };

  return { lists, archivedLists, loading, addList, updateList, archiveList, unarchiveList, deleteList, refetch: fetch };
}

// ─── HOME SETTINGS ───
export function useHomeSettings(householdId, userId) {
  const [settings, setSettings] = useState(null);
  const [loading, setLoading] = useState(true);

  const fetch = useCallback(async () => {
    if (!householdId || !userId) {
      setLoading(false);
      return;
    }
    const { data } = await supabase
      .from('home_settings')
      .select('*')
      .eq('household_id', householdId)
      .eq('user_id', userId)
      .maybeSingle();
    if (data) setSettings(data);
    setLoading(false);
  }, [householdId, userId]);

  useEffect(() => {
    fetch();
  }, [fetch]);

  const saveSettings = async (updates) => {
    const { data, error } = await supabase
      .from('home_settings')
      .upsert(
        { household_id: householdId, user_id: userId, ...updates, updated_at: new Date().toISOString() },
        { onConflict: 'household_id,user_id' },
      )
      .select()
      .single();
    if (error) {
      console.error('Errors.settingsSaveFailed', error);
      notifyError('Errors.settingsSaveFailed');
    }
    if (data) setSettings(data);
  };

  return { settings, loading, saveSettings };
}

// ─── TODO ITEMS (whole household, grouped by list — one query/channel) ───
export function useTodoItems(householdId) {
  const {
    data: items,
    loading,
    refetch,
    applyLocal,
  } = useHouseholdTable('todo_items', householdId, 'sort_order', true);
  const itemsRef = useRef(items);
  itemsRef.current = items;

  const itemsByList = useMemo(() => {
    const map = {};
    for (const item of items) (map[item.list_id] ||= []).push(item);
    return map;
  }, [items]);

  const addItem = async (listId, title) => {
    const listItems = itemsRef.current.filter((i) => i.list_id === listId);
    const maxOrder = listItems.length > 0 ? Math.max(...listItems.map((i) => i.sort_order ?? 0)) : 0;
    await run(
      supabase
        .from('todo_items')
        .insert([{ title, list_id: listId, household_id: householdId, checked: false, sort_order: maxOrder + 1 }]),
      'Errors.addFailed',
    );
    await refetch();
  };
  // Optimistic: apply locally first, write in the background (stable identity
  // via refs so memoized rows don't re-render on every list change).
  const updateItem = useCallback(
    (id, updates) => {
      applyLocal(id, updates);
      optimisticWrite(supabase.from('todo_items').update(updates).eq('id', id), id, applyLocal, 'Errors.saveFailed');
    },
    [applyLocal],
  );
  const deleteItem = useCallback(
    async (id) => {
      await run(supabase.from('todo_items').delete().eq('id', id), 'Errors.deleteFailed');
      await refetch();
    },
    [refetch],
  );
  const toggleItem = useCallback(
    (id) => {
      const item = itemsRef.current.find((i) => i.id === id);
      if (item) updateItem(id, { checked: !item.checked });
    },
    [updateItem],
  );

  return { itemsByList, loading, addItem, updateItem, deleteItem, toggleItem, refetch };
}

// ─── HOME DEVICES (Naprave) — shared across the whole household ───
// Unlike calendar_freebusy_sources, there's no per-user privacy boundary:
// a physical device belongs to the household the same way a freezer does.
// Writes never happen directly from the client — home_devices is read-only
// via RLS — they go through /api/home-devices/[id]/command, which runs the
// provider call server-side and writes the authoritative state back with
// the service role. This hook applies an optimistic local patch so toggling
// power/temp/mode feels instant, then lets realtime reconcile it once the
// server's write lands (useHouseholdTable already drops an override once
// the row matches it — see its reconciliation logic above).
export function useHomeDevices(householdId) {
  const {
    data: devices,
    loading,
    refetch,
    applyLocal,
  } = useHouseholdTable('home_devices', householdId, 'created_at', true);

  const sendCommand = useCallback(
    async (deviceId, type, value, optimisticStatePatch) => {
      if (optimisticStatePatch) {
        const device = devices.find((d) => d.id === deviceId);
        if (device) applyLocal(deviceId, { state: { ...device.state, ...optimisticStatePatch } });
      }

      const {
        data: { session },
      } = await supabase.auth.getSession();
      if (!session) {
        notifyError('Errors.deviceCommandFailed');
        applyLocal(deviceId, null);
        return false;
      }

      try {
        const res = await fetch(`/api/home-devices/${deviceId}/command`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${session.access_token}` },
          body: JSON.stringify({ type, value }),
        });
        if (!res.ok) throw new Error(`command failed: ${res.status}`);
        return true;
      } catch (err) {
        console.error('Errors.deviceCommandFailed', err);
        notifyError('Errors.deviceCommandFailed');
        applyLocal(deviceId, null);
        return false;
      }
    },
    [devices, applyLocal],
  );

  const refreshDevice = useCallback((deviceId) => sendCommand(deviceId, 'refresh'), [sendCommand]);

  return { devices, loading, refetch, sendCommand, refreshDevice };
}

// ─── NAPRAVE FAVORITES (Bližnjice) ───
// Household-level choice of up to 3 devices shown as shortcuts at the top
// of Naprave (see components/DevicesModule.js). A sparse preference table,
// same shape as useShoppingSections — full client CRUD (this is UI state,
// not device control, unlike home_devices itself which is read-only for
// clients and only ever written by the service-role command route).
export function useNapraveFavorites(householdId) {
  const { data: rows, loading, refetch } = useHouseholdTable('naprave_favorites', householdId, 'sort_order', true);

  // Persists the full explicit set + order of favorite device ids — upserts
  // every id in the new list and deletes any row no longer in it, mirroring
  // useShoppingSections.reorderSections' "always write the full order"
  // approach so a favorite dropped from the picker doesn't linger as an
  // orphaned row.
  const setFavorites = async (deviceIds) => {
    const toDelete = rows.filter((r) => !deviceIds.includes(r.device_id));
    if (toDelete.length) {
      await run(
        supabase.from('naprave_favorites').delete().in(
          'id',
          toDelete.map((r) => r.id),
        ),
        'Errors.saveFailed',
      );
    }
    if (deviceIds.length) {
      await run(
        supabase.from('naprave_favorites').upsert(
          deviceIds.map((device_id, i) => ({ household_id: householdId, device_id, sort_order: i })),
          { onConflict: 'household_id,device_id' },
        ),
        'Errors.saveFailed',
      );
    }
  };

  const favoriteIds = useMemo(() => rows.map((r) => r.device_id), [rows]);

  return { favoriteIds, loading, setFavorites, refetch };
}

// ─── PROVIDER CONNECTIONS (Naprave / home-device provider linking) ───
// Read-only for clients (see provider_connections migration) — connect()
// and disconnect() go through server routes that hold the actual
// credentials/tokens; this hook only ever sees the display-safe row
// (status/account_email/last_error), never a token.
export function useProviderConnection(householdId, providerName) {
  const { data: rows, loading, refetch } = useHouseholdTable('provider_connections', householdId, 'created_at', true);
  const connection = rows.find((r) => r.provider === providerName) || null;
  const [busy, setBusy] = useState(false);

  // credentials is spread straight into the request body, so each
  // provider's connect form passes whatever shape its connect route branch
  // expects — { email, password } for MELCloud/Vaillant, { authKey,
  // server, devices } for Shelly (see app/api/home-devices/connect/
  // route.js's isShelly branch) — without this hook needing to know the
  // difference.
  const connect = useCallback(
    async (credentials) => {
      const {
        data: { session },
      } = await supabase.auth.getSession();
      if (!session) return { ok: false, error: 'unauthorized' };

      setBusy(true);
      try {
        const res = await fetch('/api/home-devices/connect', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${session.access_token}` },
          body: JSON.stringify({ householdId, provider: providerName, ...credentials }),
        });
        const body = await res.json().catch(() => ({}));
        if (!res.ok) return { ok: false, error: body?.error || 'unknown', message: body?.message };
        refetch();
        return { ok: true, deviceSyncError: body?.deviceSyncError || null };
      } catch (err) {
        return { ok: false, error: 'unknown', message: String(err?.message || err) };
      } finally {
        setBusy(false);
      }
    },
    [householdId, providerName, refetch],
  );

  const disconnect = useCallback(async () => {
    const {
      data: { session },
    } = await supabase.auth.getSession();
    if (!session) return false;

    setBusy(true);
    try {
      const res = await fetch('/api/home-devices/disconnect', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${session.access_token}` },
        body: JSON.stringify({ householdId, provider: providerName }),
      });
      if (!res.ok) return false;
      refetch();
      return true;
    } finally {
      setBusy(false);
    }
  }, [householdId, providerName, refetch]);

  return { connection, loading, busy, connect, disconnect };
}

// ─── WEB PUSH SUBSCRIPTION ───
// applicationServerKey must be a raw Uint8Array, not the base64url string.
const urlBase64ToUint8Array = (base64url) => {
  const padding = '='.repeat((4 - (base64url.length % 4)) % 4);
  const base64 = (base64url + padding).replace(/-/g, '+').replace(/_/g, '/');
  return Uint8Array.from(atob(base64), (c) => c.charCodeAt(0));
};

export function usePushSubscription(householdId, userId, locale) {
  const [supported, setSupported] = useState(false);
  const [needsInstall, setNeedsInstall] = useState(false);
  const [permission, setPermission] = useState('default');
  const [subscribed, setSubscribed] = useState(false);
  const [busy, setBusy] = useState(false);

  useEffect(() => {
    const isIOS =
      /iPad|iPhone|iPod/.test(navigator.userAgent) ||
      (navigator.platform === 'MacIntel' && navigator.maxTouchPoints > 1);
    const standalone = window.matchMedia('(display-mode: standalone)').matches || window.navigator.standalone === true;
    // Order matters: iOS Safari has no Notification API at all until the app
    // is installed to the home screen (iOS 16.4+), so check that case first.
    if (isIOS && !standalone) {
      setNeedsInstall(true);
      return;
    }
    if (!('serviceWorker' in navigator) || !('PushManager' in window) || !('Notification' in window)) return;
    setSupported(true);
    setPermission(Notification.permission);
  }, []);

  // Reconcile on every app open (and on locale switch): an existing browser
  // subscription — possibly recreated by the SW's pushsubscriptionchange —
  // gets its DB row upserted with the current user/household/locale.
  useEffect(() => {
    if (!supported || !householdId || !userId) return;
    let cancelled = false;
    (async () => {
      const reg = await navigator.serviceWorker.ready;
      const sub = await reg.pushManager.getSubscription();
      if (cancelled || !sub) return;
      const { endpoint, keys } = sub.toJSON();
      await supabase
        .from('push_subscriptions')
        .upsert(
          { endpoint, p256dh: keys.p256dh, auth: keys.auth, user_id: userId, household_id: householdId, locale },
          { onConflict: 'endpoint' },
        );
      if (!cancelled) setSubscribed(true);
    })();
    return () => {
      cancelled = true;
    };
  }, [supported, householdId, userId, locale]);

  // Must be called from a user gesture (requestPermission requirement).
  const enable = async () => {
    if (!supported || busy) return;
    setBusy(true);
    try {
      const perm = await Notification.requestPermission();
      setPermission(perm);
      if (perm !== 'granted') return;
      const reg = await navigator.serviceWorker.ready;
      const sub = await reg.pushManager.subscribe({
        userVisibleOnly: true,
        applicationServerKey: urlBase64ToUint8Array(process.env.NEXT_PUBLIC_VAPID_PUBLIC_KEY),
      });
      const { endpoint, keys } = sub.toJSON();
      const { error } = await supabase
        .from('push_subscriptions')
        .upsert(
          { endpoint, p256dh: keys.p256dh, auth: keys.auth, user_id: userId, household_id: householdId, locale },
          { onConflict: 'endpoint' },
        );
      if (error) throw error;
      setSubscribed(true);
    } catch (e) {
      console.error('push subscribe failed', e);
      notifyError('Errors.pushSubscribeFailed');
    } finally {
      setBusy(false);
    }
  };

  const disable = async () => {
    if (busy) return;
    setBusy(true);
    try {
      const reg = await navigator.serviceWorker.ready;
      const sub = await reg.pushManager.getSubscription();
      if (sub) {
        await supabase.from('push_subscriptions').delete().eq('endpoint', sub.endpoint);
        await sub.unsubscribe();
      }
      setSubscribed(false);
    } catch (e) {
      console.error('push unsubscribe failed', e);
      notifyError('Errors.pushUnsubscribeFailed');
    } finally {
      setBusy(false);
    }
  };

  return { supported, needsInstall, permission, subscribed, busy, enable, disable };
}

// ─── BOARD NOTES (DESKA) ───
// Household sticky notes. Only active notes (done_at IS NULL) are fetched;
// marking done keeps the row but drops it from the list. Realtime keeps both
// members in sync. markDone/updateNote skip await for optimistic feel — the
// postgres_changes subscription reconciles.
export function useBoardNotes(householdId) {
  const [notes, setNotes] = useState([]); // active (done_at null)
  const [archived, setArchived] = useState([]); // done (done_at set)
  const [loading, setLoading] = useState(true);

  const fetch = useCallback(async () => {
    if (!householdId) {
      setLoading(false);
      return;
    }
    const [act, arch] = await Promise.all([
      supabase
        .from('board_notes')
        .select('*')
        .eq('household_id', householdId)
        .is('done_at', null)
        .order('created_at', { ascending: false }),
      supabase
        .from('board_notes')
        .select('*')
        .eq('household_id', householdId)
        .not('done_at', 'is', null)
        .order('done_at', { ascending: false }),
    ]);
    if (act.data) setNotes(act.data);
    if (arch.data) setArchived(arch.data);
    setLoading(false);
  }, [householdId]);

  useEffect(() => {
    fetch();
    if (!householdId) return;
    const ch = supabase
      .channel('board_notes-' + householdId)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'board_notes' }, () => fetch())
      .subscribe();
    return () => {
      supabase.removeChannel(ch);
    };
  }, [fetch, householdId]);

  // All writes go through run() — a bare query builder is a lazy thenable and
  // never fires unless awaited (this is why editing/markDone used to no-op).
  const addNote = (text, userId, authorName, eventDate) =>
    run(
      supabase.from('board_notes').insert([
        {
          household_id: householdId,
          text,
          created_by: userId,
          author_name: authorName,
          event_date: eventDate || null,
        },
      ]),
      'Errors.addFailed',
    );
  const updateNote = (id, updates) =>
    run(supabase.from('board_notes').update(updates).eq('id', id), 'Errors.saveFailed');
  const markDone = (id) =>
    run(supabase.from('board_notes').update({ done_at: new Date().toISOString() }).eq('id', id), 'Errors.saveFailed');
  const unarchiveNote = (id) =>
    run(supabase.from('board_notes').update({ done_at: null }).eq('id', id), 'Errors.saveFailed');
  const deleteNote = (id) => run(supabase.from('board_notes').delete().eq('id', id), 'Errors.deleteFailed');

  return { notes, archived, loading, addNote, updateNote, markDone, unarchiveNote, deleteNote, refetch: fetch };
}

// ─── WEATHER (Open-Meteo, free, no key) ───
// Current temp/feel/wind + a 7-day forecast, for arbitrary coordinates.
// timezone=auto (not hardcoded Ljubljana) since locations can be anywhere.
function weatherUrl(lat, lng) {
  return (
    `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lng}` +
    `&current=temperature_2m,apparent_temperature,weather_code,wind_speed_10m,precipitation_probability` +
    `&daily=weather_code,temperature_2m_max,temperature_2m_min,precipitation_probability_max,precipitation_sum` +
    `&timezone=auto&forecast_days=7`
  );
}

// One-shot fetch for a location that isn't the home card's "main" one — used
// by the weather modal when switching between location chips. Lazy by
// design: only the tapped location is ever fetched, not the whole list.
export async function fetchWeatherOnce(lat, lng) {
  try {
    const res = await fetch(weatherUrl(lat, lng));
    return await res.json();
  } catch {
    return null; // offline / API down — caller just keeps showing stale/no data
  }
}

// Home card's main location. Refreshes on mount, on lat/lng change (i.e.
// when "set as main" switches location), and every 30 min.
export function useWeather(lat = 46.05, lng = 14.51) {
  const [weather, setWeather] = useState(null);
  useEffect(() => {
    let active = true;
    const load = async () => {
      const data = await fetchWeatherOnce(lat, lng);
      if (active && data) setWeather(data);
    };
    load();
    const t = setInterval(load, 30 * 60 * 1000);
    return () => {
      active = false;
      clearInterval(t);
    };
  }, [lat, lng]);
  return weather;
}

// Open-Meteo geocoding (free, no key) — powers the "add location" search.
// Small/unlisted places (e.g. a ski resort) won't show up here; the UI falls
// back to manual lat/lng entry for those.
export async function geocodeLocation(query) {
  try {
    const res = await fetch(
      `https://geocoding-api.open-meteo.com/v1/search?name=${encodeURIComponent(query)}&count=5&language=sl`,
    );
    const data = await res.json();
    return (data.results || []).map((r) => ({
      name: r.name,
      lat: r.latitude,
      lng: r.longitude,
      label: [r.name, r.admin1, r.country].filter(Boolean).join(', '),
    }));
  } catch {
    return [];
  }
}
