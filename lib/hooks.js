...
export function useTodoLists(householdId) {
  ...
  useEffect(() => {
    fetch();
    const ch = supabase.channel('todo_lists-' + householdId)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'todo_lists' }, fetch)
      .subscribe();
    return () => { supabase.removeChannel(ch); };
  }, [fetch, householdId]);

  const addList = async (list) => supabase.from('todo_lists').insert([{ ...list, household_id: householdId }]);
  const updateList = async (id, updates) => supabase.from('todo_lists').update(updates).eq('id', id);
  const archiveList = async (id) => supabase.from('todo_lists').update({ archived_at: new Date().toISOString() }).eq('id', id);
  const deleteList = async (id) => supabase.from('todo_lists').delete().eq('id', id);
...
export function useTodoArchivedLists(householdId) {
  ...
  useEffect(() => {
    fetch();
    const ch = supabase.channel('todo_lists_archived-' + householdId)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'todo_lists' }, fetch)
      .subscribe();
    return () => { supabase.removeChannel(ch); };
  }, [fetch, householdId]);
...
export function useTodoItems(householdId, listId) {
  ...
  useEffect(() => {
    fetch();
    const ch = supabase.channel('todo_items-' + listId)
  ...
  const updateItem = async (id, updates) => supabase.from('todo_items').update(updates).eq('id', id);
  const deleteItem = async (id) => supabase.from('todo_items').delete().eq('id', id);
