...
export function analyzeShoppingHistory(archivedItems, refMonth = new Date(), avgRangeMonths = null) {
  ...
  // Compare this month against a rolling baseline built from prior months
  // that actually have reliable amount data — not a fixed target, since
  // real spend legitimately varies month to month. Months before
  // amount-tracking started (or any month with zero priced purchases)
  // contribute nothing here, so they can't drag a false "average" down.
  // `avgRangeMonths` narrows how far back that baseline looks (null = since
  // tracking started, i.e. every prior month with data); it mirrors the
  // "Average" card's own range picker in the UI.
  const monthIndex = (d) => d.getFullYear() * 12 + d.getMonth();
  const refIndex = monthIndex(monthStart);
  const minIndex = avgRangeMonths ? refIndex - avgRangeMonths : -Infinity;
  const priorByMonth = {}; // monthIndex -> { total, count, items }
  for (const g of allGroups) {
    if (!g.date || g.amount == null) continue;
    const idx = monthIndex(g.date);
    if (idx >= refIndex || idx < minIndex) continue; // only strictly earlier months, within range
    if (!priorByMonth[idx]) priorByMonth[idx] = { total: 0, count: 0, items: 0 };
    priorByMonth[idx].total += g.amount;
    priorByMonth[idx].count += 1;
    priorByMonth[idx].items += g.items.length;
  }
  const priorMonths = Object.values(priorByMonth);
  const avgMonthsCount = priorMonths.length;
  const priorTotalSpend = priorMonths.reduce((s, m) => s + m.total, 0);
  const priorTotalPurchases = priorMonths.reduce((s, m) => s + m.count, 0);
  const priorTotalItems = priorMonths.reduce((s, m) => s + m.items, 0);
  const avgMonthlySpend = avgMonthsCount ? priorTotalSpend / avgMonthsCount : null;
  // The "Average" card's own trio, mirroring the month card's (count, avg
  // price per purchase, avg items per purchase) — computed across all
  // qualifying purchases pooled together, not as an average of per-month
  // averages, since that's more robust with only a handful of months.
  const avgPurchasesPerMonth = avgMonthsCount ? Math.round(priorTotalPurchases / avgMonthsCount) : null;
  const avgBasketAcrossPrior = priorTotalPurchases ? priorTotalSpend / priorTotalPurchases : null;
  const avgItemsAcrossPrior = priorTotalPurchases ? Math.round(priorTotalItems / priorTotalPurchases) : null;
