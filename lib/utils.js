...
// Deterministic fallback for a member who hasn't picked a color in Settings —
// without this every unset member falls back to the same gray dot/bar
// (memberColorClass returns undefined for a null token), so nobody is visually
// distinguishable. Order is by household_members.id (stable, never changes),
// so a given member always lands on the same fallback color across renders
// and screens, and an explicit m.color always wins over the fallback.
export function memberColorToken(members, member) {
  if (!member) return null;
  if (member.color) return member.color;
  const ids = [...members].map((m) => m.id).sort();
  const idx = ids.indexOf(member.id);
  return MEMBER_COLORS[(idx < 0 ? 0 : idx) % MEMBER_COLORS.length]?.t;
}
...
// Fallback shown until the user has saved any weather location (or if the
// column is empty for some other reason) — keeps the home card populated
// out of the box instead of rendering nothing.
export const DEFAULT_WEATHER_LOCATIONS = [{ name: 'Ljubljana', lat: 46.05, lng: 14.51 }];

// home_settings.weather_locations, with the default applied. Order matters:
// index 0 is always the "main" location shown on the home card.
export function weatherLocationsOf(settings) {
  const locs = settings?.weather_locations;
  return Array.isArray(locs) && locs.length ? locs : DEFAULT_WEATHER_LOCATIONS;
}
